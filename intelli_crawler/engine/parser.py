...
class Parser:
    ...
    def extract_list_records(
        self, source: SourceConfig, html: str, base_url: str
    ...
    def extract_xueqiu_records(self, html: str, base_url: str) -> dict[str, dict[str, Any]]:
        """
        专门处理雪球网站的时间线内容提取
        从整个时间线文本中按行解析新闻条目，支持动态加载内容
        """
        parser = HTMLParser(html)
        timeline_element = parser.css_first(".style_home__timeline_1Tz")

        if not timeline_element:
            return {}

        # 获取时间线的文本内容
        timeline_text = timeline_element.text(separator="\n", strip=True)
        ...
        for line in lines:
            ...
            # 跳过导航标签和无关内容
            if line in ["热门", "7x24", "视频", "基金", "资讯", "达人", "私募", "ETF"]:
                continue

            # 跳过日期行（如 "17十月"）
            if re.match(r"^\d+[十月年]", line):
                continue
            ...
            if time_pattern.match(line):
                ...
                # 添加到当前项目
                if current_item:  # 只有在有时间戳开始后才添加内容
        ...
        for i, item in enumerate(news_items):
            ...
            records[record_url] = {
                "title": content_part[:100] + "..." if len(content_part) > 100 else content_part,
